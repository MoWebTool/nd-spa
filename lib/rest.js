...
module.exports = Base.extend({
  ...
  request: function(options) {
    ...
    if (auth.isLogin()) {
      ...
      options.headers.Authorization = auth.getAuthentization(options.type, matched[2], matched[1])
    }
    ...
    .forEach(function(key) {
      if ((value = this.get(key))) {
        if (Array.isArray(value)) {
          ...
          // post的data数据不一定都是keyvalue，可以直接是数组，例如：4.1.17 [POST]/users/actions/query?realm=xxx 批量获取用户信息
