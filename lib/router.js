...
function asyncRequire(obj) {
  ...
  progress.show();
  pendingRequire = true;
  // require('app/' + obj.app + '/index.js', function(bootstrap) {
  require.ensure([], function(require) {
    pendingRequire = false;

    // todo: is there a clear way for check 404?
    try {
...
function startup(obj, undef) {
  ...
  asyncRender(undef, obj.app, isLogin);
...
router.goLast = function() {
  if (lastRoute) {
    router.redirect(lastRoute);
  }
};

router.goNext = function(yes) {
  if (!yes) {
    cancelNext = null;
    nextRoute = null;
  }
